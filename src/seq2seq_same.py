...
class LSTMSeq2SeqSame(nn.Module):
    ...
    def forward(self, x, hidden_state, cell_state):
        output, (hidden_state, _) = self.lstm(x, hidden_state, cell_state)
        ...
        output = self.fc(output.view(-1, output.shape[-1]))  # fc computation for each element
        output = output.view(orig_dims[0], orig_dims[1], output.shape[1])  # reshaping to have (seq_len, batch, output)
        return output
...
class BaselineSeq2SeqSame(nn.Module):
    """ Seq2Seq scenario where input and output are of the same length - PyTorch LSTM implementation
    """
    def __init__(self, n_input, n_hidden, n_output, layers=1, bidirectional=False, layernorm=False):
        super().__init__()
    ...
    def forward(self, x, h, c):
        o, (h, _) = self.lstm(x, (h, c))
        orig_dims = o.shape
        if self.layernorm:
            o = self.ln(o.transpose(0,1))  # layernorm input should be (batch, *)
        output = self.fc(o.view(-1, o.shape[-1]))  # fc computation for each element
        output = output.view(orig_dims[0], orig_dims[1], output.shape[1])  # reshaping to have (seq_len, batch, output)
        return output
    ...
    def train(self, epochs, train_loader, valid_loader=None, freq=10, out_dir='./', create_dir=True):
        ...
        for i in range(1, epochs+1):
            ...
            for j, (text, label) in enumerate(train_loader, start=1):
                ...
                start = time.time()
                hidden_state = torch.zeros(1, label.shape[0],
                                           self.model.hidden_dim, requires_grad=True).to(self.device)
                cell_state = torch.zeros(1, label.shape[0],
                                         self.model.hidden_dim, requires_grad=True).to(self.device)
                ...
                loss = self.loss_criterion(output.contiguous().view(-1, output.shape[-1]),
                                           label.contiguous().view(-1))  # reshape to have (N, classes)
                loss.backward()
                self.optimizer.step()
...
#                                                loss.item(), time.time() - start), end='\r')
        ...
        self.plot_history(self.stats['train_score'], self.stats['valid_score'], stats='accuracy',
                          file_path=os.path.join(out_dir, "accuracy_{}.png".format(i)))
    ...
    def evaluate(self, test_loader, verbose=True):
        ...
        with torch.no_grad():
            for text, label in test_loader:
                ...
                # get label predictions - since we only care about the predictions from positions which has a label (i.e., non '_')
                label_ids = label.nonzero().contiguous().view(-1)
